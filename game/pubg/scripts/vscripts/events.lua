...
function COverthrowGameMode:OnGameRulesStateChange()
	...
	if nNewState == DOTA_GAMERULES_STATE_PRE_GAME then
		...
		if GetMapName() == "dota_solo" then
			self.TEAM_KILLS_TO_WIN = 45
		elseif GetMapName() == "dota_duo" then
			self.TEAM_KILLS_TO_WIN = 90
		elseif GetMapName() == "dota_zombie_night" then
			self.TEAM_KILLS_TO_WIN = 30
		elseif GetMapName() == "dota_zombie_survival" then
			self.TEAM_KILLS_TO_WIN = PlayerResource:GetPlayerCount()*5
			...
			if event.entindex_inflictor ~= nil then
				...
				if inflictor_index ~= nil then
					...
					if ability ~= nil then
						...
						local effect = "particles/items2_fx/refresher.vpcf"
						local pfx = ParticleManager:CreateParticle(effect, PATTACH_ABSORIGIN_FOLLOW, hero)
						ParticleManager:SetParticleControl(pfx, 0, hero:GetAbsOrigin()) -- Origin
						EmitSoundOn("DOTA_Item.Refresher.Activate",hero)
--						hero:SetHealth(hero:GetHealth()+hero:GetMaxHealth()*0.4)
--						hero:SetMana(hero:GetMana()+hero:GetMaxMana()*0.4)
						for i=0,16 do
...
function COverthrowGameMode:SetRespawnTime( killedTeam, killedUnit, extraTime )
	...
	if killedTeam == self.leadingTeam and self.isGameTied == false then
		killedUnit:SetTimeUntilRespawn( 6 + extraTime )
	else
